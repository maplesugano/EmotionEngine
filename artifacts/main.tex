...
The project is divided into two major periods: a research phase to design the emotion code, and an implementation phase to build the synaesthetic emotion engine around that code.
\begin{enumerate}
    \item \textbf{Create dataset: excite model's emotions with diverse utterances and accumulate internal traces} 
    
    We hypothesise that model’s internal traces are elicited by phrases like ``Great job!'', ``Amazing!'' contains shapes of \textbf{joy-ness} that are independent of vocabulary and language. Concretely, we will create a large set of contrastive prompts that express the same emotion with different phrasings and across different languages, and we will accumulate the \textbf{reasoning traces} generated inside the model during inference.
    \item \textbf{Discover primitive emotion basis vectors and steering vectors; design the emotion code} 

    We will apply methods such as mean differences, PCA, and NMF to the collected traces to orthogonalise highly correlated directions and derive a primitive emotion basis. This rests on the assumption that an emotion like ``joy'' is, in fact, a composite of lower-level, primitive central cognitions. In other words, pre-verbal feelings may be representable as linear combinations of these primitive emotion basis vectors. We define this representation as the \textbf{emotion code}. For example, the ``deeply resonant nostalgia'' one feels when listening to a certain piece of music might be modelled as a combination like \verb|joy_0 + joy_1 + sad_4|. Constructed in this way, the emotion code functions as an equivalent representation that transcends vocabulary boundaries across languages and modalities.
    \item \textbf{Implement the emotion-code-centred Emotion Engine} \par
    We would like to explore the technical potential of the emotion-code-centred Emotion Engine; is it possible to steer model outputs towards a certain emotion code? 

    To make the final system concrete and testable, the Emotion Engine will be exposed through an interactive user interface inspired by a DJ turntable. Rather than specifying emotions through discrete labels or textual instructions, users will directly manipulate a continuous emotion space in a tactile and intuitive manner.

    When a user inputs a piece of text, the system first extracts its underlying emotional state and projects it as a direction in a low-dimensional affective space based on the pleasure–arousal–dominance (PAD) axes. This emotional vector is visualised in real time on the lower part of the interface, allowing users to see how the input text is situated in relation to core affective dimensions such as positivity, intensity, and control.

    Crucially, the user can then interactively modify this emotion vector. By dragging the point within the PAD space or adjusting dedicated controls (analogous to turning knobs or sliders on a DJ console), the user can adjust specific affective components. These manipulations directly alter the underlying emotion code before language generation occurs.

    The modified emotion code is subsequently applied as a steering signal during text generation, producing an output whose emotional tone reflects the user’s adjustments. This design allows users to explore how subtle changes in latent emotional representation translate into perceptible differences in generated language, offering an intuitive and continuous form of emotional control that is difficult to achieve with prompt-based methods.
\end{enumerate}
